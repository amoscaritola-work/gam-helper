...
clear
echo "##################################################################"
echo "Gam Helper Version 2.0"
echo "https://github.com/amoscaritola-work"
...
ChromebookUsageReport() {
  gam print cros fields status,serialNumber,annotatedAssetId,model,annotatedUser,recentUsers,activeTimeRanges,lastsync listlimit 1 > ~/Desktop/GamHelper/ChromebookUsageReport.csv
  open ~/Desktop/GamHelper/ChromebookUsageReport.csv
  ContinueOrExit
}
...
while [[ $userChoice != "5" ]]; do
  ...
  echo ""
	...
	echo "[3] Batch update info on chromebooks"
	echo "[4] Deprovision chromebooks"
  echo "[5] Save list of Chromebooks with recently logged in users"
  echo "-------------------------------------------------------------"
  echo "[u] Update GAM"
  echo "[q] Exit"
  echo ""
    ...
    echo "Getting Report of recent Chromebook activity"
    ChromebookUsageReport
  elif [ $userChoice == "q" ]; then
    echo "Exiting Script"
    exit 0
  elif [ $userChoice == "u" ]; then
    echo "Copy/Paste the following command into terminal"
    echo "bash <(curl -s -S -L https://git.io/install-gam) -l"
    exit 0
	else
